...
const CashmereWool = ({ setCounter }) => {
    const [base, setBase] = useState([]);
    const categories = ["All", "Chiffon", "CashmereWool"];
  ...
  const [view, setView] = useState({});
    ...
    useEffect(() => {
        product("CashmereWool");
    }, [isopen])

    const product = async (category) => {
        try {
            const url =
                category === "All"
    ...
    return (
        <div>
            <div className="grid grid-cols-12 gap-5 py-[50px]">
                ...
                {
                    base.map((item, index) => (
                        <div className='lg:col-span-6 md:col-span-6 col-span-12' key={index}  >
