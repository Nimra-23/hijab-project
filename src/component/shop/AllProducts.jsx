...
const AllProducts = ({ setCounter }) => {
    ...
    return (
        <div>
            <div className="grid grid-cols-12 gap-5 py-[50px]">
                {
                    base.map((base, index) => (
                        <div className='lg:col-span-3 md:col-span-6 col-span-12' key={index}  >
                            <div className='MainCard relative'>
                                ...
                                <div className="p-4    bg-white card ">
                                    <h3 className='pb-2 text-[15px]'>{base.title}</h3>
                                    ...
                                    <p className='pb-4 text-[11px] flex gap-2'>RS {base.price}<s className=' text-gray-400 '>RS 700</s></p>
                                    <div className='  AddtoCart'>
                                        <span className='font-[sans-serif] text-[12px]'>ADD TO CART</span>

                                        <button onClick={() => { handleopen(base.id) }}><i className="fa-solid fa-eye" style={{ color: "#050505" }} ></i></button>
                                        <i
                                            className={`fa-solid fa-heart cursor-pointer `}
                                            style={{ color: likedProducts.includes(base.id) ? 'red' : 'black' }}
                                            onClick={() => handleHeartClick(base.id)}
                                        ></i>
